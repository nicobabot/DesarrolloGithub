...
update_status ModuleCamera3D::Update()
{
	...
	{
		App->camera->Position.y += 0.01f, 0.0f;
	}
	...
	if(App->input->GetMouseButton(SDL_BUTTON_RIGHT) == KEY_REPEAT)
	{
		...
		Z = rotate(Z, 1*dx, vec3(0,1,0));
		X = rotate(X, 1*dx, vec3(0, 1, 0));
		Y = rotate(Y, 1 * dx, vec3(0, 1, 0));
		Z = rotate(Z,1*dy,X);
		Y = rotate(Y, 1 * dy, X);
