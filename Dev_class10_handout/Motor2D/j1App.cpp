...
bool j1App::Awake()
{
	...
	if(config.empty() == false)
	{
		...
		framerate = app_config.child("framerate").attribute("value").as_int();
	}
...
void j1App::FinishUpdate()
{
	...
	sprintf_s(title, 256, "Av.FPS: %.2f Last Frame Ms: %u Last sec frames: %i  dt: %f  Time since startup: %.3f Frame Count: %lu ",
			  avg_fps, last_frame_ms, frames_on_last_update, dt,seconds_since_startup, frame_count);
	App->win->SetTitle(title);

	// TODO 2: Use SDL_Delay to make sure you get your capped framerate
	delaytimer.Start();
	SDL_Delay((1000.f/framerate)-last_frame_ms);
	timedelayed=delaytimer.ReadMs();
	LOG("Time delayed %f", timedelayed);
